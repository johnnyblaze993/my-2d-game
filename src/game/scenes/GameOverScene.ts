...
export default class GameOverScene extends Phaser.Scene {
  ...
  create() {
    ...
    const restartButton = this.add.text(400, 300, "Restart", { fontSize: "32px", color: "#fff" })
      .setOrigin(0.5)
      .setInteractive();
    ...
    restartButton.on("pointerdown", () => {
      ...
      mainScene.events.once("create", () => {
        mainScene.startTime = mainScene.time.now; // Ensure the timer fully resets
      });
      mainScene.scene.restart(); // Properly restart MainScene
    });
    ...
    bestTimes.slice(0, 5).forEach((time: number, index: number) => {
      ...
      this.add.text(400, 450 + index * 30, `${index + 1}. ${minutes}:${seconds}`, { fontSize: "24px", color: "#fff" })
        .setOrigin(0.5);
    });
  }
}
